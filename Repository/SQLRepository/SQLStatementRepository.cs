                                            ...
                                            if (lst.Count > 0)
                                            {
                                                ...
                                                for (int x = 0; x < mergedlst.Count; x++)
                                                {
                                                    ...
                                                    if (!mergedlst[x].IsDynamicWidget)
                                                    {
                                                        ...
                                                        else if (mergedlst[x].WidgetName == HtmlConstants.PPS_DETAILED_TRANSACTIONS_WIDGET_NAME)
                                                        {
                                                            string widgetId = "PageWidgetId_" + mergedlst[x].Identifier + "_Counter" + counter.ToString(); 
                                                            //string widgetHTML = HtmlConstants.PAYMENT_SUMMARY_WIDGET_HTML_FOR_STMT.Replace("{{VideoSource}}", "{{VideoSource_" + statement.Identifier + "_" + page.Identifier + "_" + mergedlst[x].Identifier + "}}");
                                                            string widgetHTML = HtmlConstants.PPS_DETAILED_TRANSACTIONS_WIDGET_HTML_FOR_STMT;
                                                            widgetHTML = widgetHTML.Replace("{{WidgetDivHeight}}", divHeight);
                                                            widgetHTML = widgetHTML.Replace("{{WidgetId}}", widgetId);
                                                            pageHtmlContent.Append(widgetHTML);
                                                        }
                                                        ...
                                                        {
                                                            string widgetId = "PageWidgetId_" + mergedlst[x].Identifier + "_Counter" + counter.ToString();
                                                            string widgetHTML = HtmlConstants.PPS_DETAILS1_WIDGET_HTML_FOR_STMT;
                                                            widgetHTML = widgetHTML.Replace("{{WidgetDivHeight}}", divHeight);
                                                            widgetHTML = widgetHTML.Replace("{{WidgetId}}", widgetId);
                                                            pageHtmlContent.Append(widgetHTML);

                                                        }
                                                        else if (mergedlst[x].WidgetName == HtmlConstants.PPS_DETAILS2_WIDGET_NAME)
                                                        {
                                                            string widgetId = "PageWidgetId_" + mergedlst[x].Identifier + "_Counter" + counter.ToString();
                                                            string widgetHTML = HtmlConstants.PPS_DETAILS2_WIDGET_HTML_FOR_STMT;
                                                            widgetHTML = widgetHTML.Replace("{{WidgetDivHeight}}", divHeight);
                                                            widgetHTML = widgetHTML.Replace("{{WidgetId}}", widgetId);
                                                            pageHtmlContent.Append(widgetHTML);
                                                        ...
                                                        else if (mergedlst[x].WidgetName == HtmlConstants.EARNINGS_FOR_PERIOD_WIDGET_NAME)
                                                        {
                                                            string widgetId = "PageWidgetId_" + mergedlst[x].Identifier + "_Counter" + counter.ToString();
                                                            //string widgetHTML = HtmlConstants.PAYMENT_SUMMARY_WIDGET_HTML_FOR_STMT.Replace("{{VideoSource}}", "{{VideoSource_" + statement.Identifier + "_" + page.Identifier + "_" + mergedlst[x].Identifier + "}}");
                                                            string widgetHTML = HtmlConstants.EARNINGS_FOR_PERIOD_WIDGET_HTML_FOR_STMT;
                                                            widgetHTML = widgetHTML.Replace("{{WidgetDivHeight}}", divHeight);
                                                            widgetHTML = widgetHTML.Replace("{{WidgetId}}", widgetId);
                                                            pageHtmlContent.Append(widgetHTML);
                                                        }
                ...
                for (int i = 0; i < statement.Pages.Count; i++)
                {
                    ...
                    for (int j = 0; j < pagewidgets.Count; j++)
                    {
                               ...
                               string transactionListJson = "[{'INT_EXT_REF':'2164250','Int_Name':'Mr SCHOELER','Client_Name':'Kruger Van Heerden','Member_Ref':124556686,'Policy_Ref':5596100,'Description':'Safe Custody Service Fee','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'17.55','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'Safe Custody Fee'},{'INT_EXT_REF':'2164250','Int_Name':'Yvonne Van Heerden','Client_Name':'Mr SCHOELER','Member_Ref':124556686,'Policy_Ref':'5596100','Description':'Safe Custody Service Fee VAT','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'2.63','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'Safe Custody Fee'},{'INT_EXT_REF':'124411745','Int_Name':'Kruger Van Heerden','Client_Name':'DR N J Olivier','Member_Ref':'1217181','Policy_Ref':'5524069','Description':'Safe Custody Service Fee','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'17.55','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'Safe Custody Fee'},{'INT_EXT_REF':'124411745','Int_Name':'Kruger Van Heerden','Client_Name':'DR N J Olivier','Member_Ref':'124556686','Policy_Ref':'5596100','Description':'Safe Custody Service Fee VAT','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'2.63','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'VAT'}]";
                                ...
                                if (transactionListJson != string.Empty && validationEngine.IsValidJson(transactionListJson))
                                {
                                    ...
                                    records?.ForEach(transactionitem =>
                                    {
                                        ...
                                        detailedTransactionSrc.Append("<tr> <td align='center' valign='center' class='px-1 py-1 fsp-bdr-right fsp-bdr-bottom'></td> <td class='fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='text-right fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='text-left fsp-bdr-right fsp-bdr-bottom px-1 py-1'><br /></td> <td class='text-left fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='text-left fsp-bdr-right fsp-bdr-bottom px-1 py-1'>" + TotalPostedAmount + "</td> <td class='text-left fsp-bdr-bottom px-1'><a href='https://www.google.com/' target = '_blank' ><img src='assets/images/leftarrowlogo.png'></a></td> </tr></table><div class='text-right w-100 pt-3'><a href='https://www.google.com/' target = '_blank'></a></div></div></div></div>");
                                        TotalPostedAmount = 0;
                                    });
                                    pageContent.Replace("{{detailedTransaction}}", detailedTransactionSrc.ToString());
                                }
                            }
                            else if (widget.WidgetName == HtmlConstants.PPS_DETAILED_TRANSACTIONS_WIDGET_NAME)
                            {
                      ...
                      "<td class='bdr-right-white text-left'>" + memberitemrecord.POLICY_REF + "</td><td class='bdr-right-white text-left'>" + memberitemrecord.PRODUCT_DESCRIPTION + "</td><td class='bdr-right-white text-left'>" + memberitemrecord.REQUEST_DATETIME.ToString("dd-MMM-yyyy") + "</td><td class='bdr-right-white text-left'>" + memberitemrecord.REQUESTED_DATETIME.ToString("dd-MMM-yyyy") + "</td><td class='bdr-right-white text-left'>" + memberitemrecord.CommissionType + "</td><td class='bdr-right-white text-left'>" + Utility.FormatCurrency(memberitemrecord.TRANSACTION_AMOUNT) + "</td><td class='bdr-right-white text-right'>" + memberitemrecord.AE_Posted_Date.ToString("dd-MMM-yyyy") + "</td>" +
                            ...
                            else if (widget.WidgetName == HtmlConstants.EARNINGS_FOR_PERIOD_WIDGET_NAME)
                            {
                                    ...
                                    if (commisionDetailListJson != string.Empty && validationEngine.IsValidJson(commisionDetailListJson))
                                    {
                                        ...
                                        {
                                            for (int x = 0; x < numFeesColumns; x++)
                                            {
                                                FeesColumnSums[x] += row[x];
                                            }
                                        }
                                        ...
                                        fspNameRecords.ForEach(gpfspNameItem =>
                                        {
                                            ...
                                            commisionDetailSrc.Append("<td class='text-right font-weight-bold'>" + totalAEPostedMonthlyEarningsR + "</ td>");
                                            commisionDetailSrc.Append("</tr></table></div></div>");
                        ...
                        if (!widget.IsDynamicWidget)
                        {
                               ...
                               string transactionListJson = "[{'INT_EXT_REF':'2164250','Int_Name':'Mr SCHOELER','Client_Name':'Kruger Van Heerden','Member_Ref':124556686,'Policy_Ref':5596100,'Description':'Safe Custody Service Fee','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'17.55','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'Safe Custody Fee'},{'INT_EXT_REF':'2164250','Int_Name':'Yvonne Van Heerden','Client_Name':'Mr SCHOELER','Member_Ref':124556686,'Policy_Ref':'5596100','Description':'Safe Custody Service Fee VAT','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'2.63','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'Safe Custody Fee'},{'INT_EXT_REF':'124411745','Int_Name':'Kruger Van Heerden','Client_Name':'DR N J Olivier','Member_Ref':'1217181','Policy_Ref':'5524069','Description':'Safe Custody Service Fee','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'17.55','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'Safe Custody Fee'},{'INT_EXT_REF':'124411745','Int_Name':'Kruger Van Heerden','Client_Name':'DR N J Olivier','Member_Ref':'124556686','Policy_Ref':'5596100','Description':'Safe Custody Service Fee VAT','Commission_Type':'Safe Custody Fee','POSTED_DATE':'20-Mar-23','Display_Amount':'2.63','Query_Link':'https://www.google.com/','TYPE':'Fiduciary_Data','Prod_Group':'VAT'}]";
                                ...
                                if (transactionListJson != string.Empty && validationEngine.IsValidJson(transactionListJson))
                                {
                                    ...
                                    records?.ForEach(transactionitem =>
                                    {
                                        ...
                                        string TotalPostedAmountR = (TotalPostedAmount == 0) ? "R0.00" : Utility.FormatCurrency(TotalPostedAmount.ToString());
                                        detailedTransactionSrc.Append("<tr> <td align='center' valign='center' class='px-1 py-1 fsp-bdr-right fsp-bdr-bottom'></td> <td class='fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='text-right fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='text-left fsp-bdr-right fsp-bdr-bottom px-1 py-1'><br /></td> <td class='text-left fsp-bdr-right fsp-bdr-bottom px-1 py-1'></td> <td class='text-right fsp-bdr-right fsp-bdr-bottom px-1 py-1'>" + TotalPostedAmountR + "</td> <td class='text-left fsp-bdr-bottom px-1'><a href='https://www.google.com/' target = '_blank' ><img src='assets/images/leftarrowlogo.png'></a></td> </tr></table><div class='text-right w-100 pt-3'><a href='https://www.google.com/' target = '_blank'></a></div></div></div></div>");
                                        TotalPostedAmount = 0;
                                    });
                                    pageContent.Replace("{{detailedTransaction}}", detailedTransactionSrc.ToString());
                                }
                            ...
                            else if (widget.WidgetName == HtmlConstants.EARNINGS_FOR_PERIOD_WIDGET_NAME)
                            {
                                    ...
                                    if (commisionDetailListJson != string.Empty && validationEngine.IsValidJson(commisionDetailListJson))
                                    {
                                        ...
                                        foreach (var row in monthlyFessTotalList)
                                        {
                                            for (int m = 0; m < numFeesColumns; m++)
                                            {
                                                FeesColumnSums[m] += row[m];
                                            }
                                        }
                                        ...
                                        if (productSummary != null && productSummary.Count > 0)
                                        {
                                            ...
                                            gpCommissionTypeRecords.ForEach(gpCommissionTypeItem =>
                                            {
                                                ...
                                                prdocutDescriptionRecords.ForEach(gpPrdocutDescriptionItem =>
                                                {
                                                    ...
                                                    // Append the table row to productSummarySrc
                                                    productSummarySrc.Append("<tr><td align='center' valign='center' class='px-1 py-1 fsp-bdr-right fsp-bdr-bottom'>" + index + "</td><td class='fsp-bdr-right text-left fsp-bdr-bottom px-1'>" + gpCommissionTypeItem.Key.Commission_Type + "</td>" + "<td class='fsp-bdr-right text-left fsp-bdr-bottom px-1'> " + (gpPrdocutDescriptionItem.Key.Prod_Group == "Service Fee" ? "Premium Under Advise Fee" : gpPrdocutDescriptionItem.Key.Prod_Group) + "</td> <td class='text-right fsp-bdr-right fsp-bdr-bottom px-1'>" + Utility.FormatCurrency(aeAmountSumR) + "</td><td class='text-left fsp-bdr-bottom px-1'><a  href ='https://www.google.com/' target='_blank'><img class='leftarrowlogo' src ='assets/images/leftarrowlogo.png' alt = 'Left Arrow'></a></td></tr>");
