...
namespace nIS
{
    public class HtmlConstants
    {
        ...
        "<a class='navbar-brand ml-auto' href='#'> <img src = 'absa-logo.PNG' height='60'></a> </nav> " +
        "<nav class='navbar navbar-expand-sm bg-dark navbar-dark'><div class='collapse navbar-collapse' id='collapsibleNavbar'>" +
        "<ul class='navbar-nav nav'>" +
            "<li class='nav-item'><a class='nav-link active' data-toggle='tab' href='#"+ HOME_PAGE_DIV_NAME + "'>At a Glance</a> </li>" +
            "<li class='nav-item'> <a class='nav-link' data-toggle='tab' href='#"+ SAVING_ACCOUNT_PAGE_DIV_NAME + "'>Saving Account</a></li> " +
            "<li class='nav-item'><a class='nav-link' data-toggle='tab' href='#"+CURRENT_ACCOUNT_PAGE_DIV_NAME+"'>Current Account</a></li>" +
            "<li class='nav-item'><a class='nav-link' href='#'>Home Loan</a></li>" +
            "<li class='nav-item'><a class='nav-link' href='#'>Contact Us</a></li>" +
            "</ul>" + "<ul class='navbar-nav ml-auto'>" +
             "<li class='nav-item date'><a class='text-white'>31<sup>st</sup> MAY 2020</a></li>" + "</ul></div></nav>";

        public const string CUSTOMER_INFORMATION_WIDGET_HTML = "<div class='card border-0 mb-3 card-shadow'>" +
                        "<div class='card-header bg-light border-0'><h5 class='m-0'>Customer Information</h5></div>" +
                        "<div class='card-body'><div class='row'><div class='col-sm-4'><h4 class='mb-4'>{{CustomerName}}</h4>" +
                        "<h6>{{Address1}}<br /> {{Address2}}</h6></div>" +
                        "<div class='col-sm-8'> <video class='doc-video'  style='width:100%' controls><source src='{{VideoSource}}' type='video/mp4'></video>" +
                        "</div></div></div></div>";

        public const string ACCOUNT_INFORMATION_WIDGET_HTML = "<div class='card border-0 mb-3 card-shadow'>" +
                       "<div class='card-header bg-light border-0'><h5 class='m-0'>Account Information</h5>" +
        ...
        public const string SUMMARY_AT_GLANCE_WIDGET_HTML = "<div class='card border-0 card-shadow'>" +
                        "<div class='card-header bg-light border-0'> <h5 class='m-0'>Summary at Glance</h5></div>" +
        ...
        public const string IMAGE_WIDGET_HTML = "<div class='card border-0 mb-3 card-shadow'>" +
                        "<div class='card-header bg-light border-0'><h5 class='m-0'>Image Information</h5></div>" +
                        "<div class='card-body text-center'><img src='{{ImageSource}}' class='img-fluid'/></div></div>";

        public const string VIDEO_WIDGET_HTML = "<div class='card border-0 mb-3 card-shadow'>" +
                        "<div class='card-header bg-light border-0'><h5 class='m-0'>Video Information</h5> </div><div class='card-body text-center'>" +
                        "<video class='doc-video' style='width:100%' controls><source src='{{VideoSource}}' type='video/mp4'></video>" +
                        "</div></div>";

        public const string NO_WIDGET_MESSAGE_HTML = "<div class='card border-0 mb-3 card-shadow'>" +
                        "<div class='card-header bg-light border-0'><h5 class='m-0'>No Configuration</h5> </div><div class='card-body text-center text-danger'>" +
                        "<span>No configuration saved for this record.</span>" +
                        "</div></div>";

        public const string CONTAINER_DIV_HTML_HEADER = "<div class='container-fluid mt-3 mb-3'>";

        public const string WIDGET_HTML_HEADER = "<div class='card border-0 p-2 tabDivClass' id='{{DivId}}'>";

        public const string WIDGET_HTML_FOOTER = "</div> ";

        public const string CONTAINER_DIV_HTML_FOOTER = "</div>";

        public const string HOME_PAGE_DIV_NAME = "HomeDiv";

        public const string SAVING_ACCOUNT_PAGE_DIV_NAME = "SavingAccDiv";

        public const string CURRENT_ACCOUNT_PAGE_DIV_NAME = "CurrentAccDiv";
    }
}
