...
import { RenderEngine } from 'src/app/layout/render-engine/renderengine.model';
...
export class RenderengineService {
  ...
  async getRenderEngines(searchParameter): Promise<RenderEngine[]> {
    ...
    return <RenderEngine[]>this.renderEngines
  }

  //service method to save or update render engine records
  public async saveRenderEngine(postData, renderEngineEditModeOn): Promise<boolean> {
    let httpClientService = this.injector.get(HttpClientService);
  ...
  public async deleteRenderEngine(postData): Promise<boolean> {
    let httpClientService = this.injector.get(HttpClientService);
  ...
  public async deactivateRenderEngine(postData): Promise<boolean> {
    let httpClientService = this.injector.get(HttpClientService);
    let identifier = null;
  ...
  public async activateRenderEngine(postData): Promise<boolean> {
    let httpClientService = this.injector.get(HttpClientService);
    let identifier = null;
    if (postData.length > 0) {
        identifier = postData[0].Identifier;
