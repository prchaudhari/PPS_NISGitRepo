                                        ...
                                        if (!mergedlst[i].IsDynamicWidget)
                                        {
                                            ...
                                            else if (mergedlst[i].WidgetName == HtmlConstants.DETAILED_TRANSACTIONS_WIDGET_NAME)
                                            {
                                                ...
                                                if (transactionListJson != string.Empty && validationEngine.IsValidJson(transactionListJson))
                                                {
                                                    ...
                                                    {
                                                        detailedTransactionSrc.Append("<tr><td class='px-1 py-1 fsp-bdr-right fsp-bdr-bottom text-left'>" + item.Client_Name + "</td><td class= 'fsp-bdr-right fsp-bdr-bottom px-1 text-nowrap text-left'>" + item.Member_Ref + "</td><td class= 'fsp-bdr-right fsp-bdr-bottom px-1 text-nowrap text-left'> " + item.Policy_Ref + "</td><td class= 'text-left fsp-bdr-right fsp-bdr-bottom px-1'>" + (item.Description == "Commission Service Fee" ? "Premium Under Advise Fee" : item.Description) + "</td><td class= 'text-left fsp-bdr-right fsp-bdr-bottom px-1'>" + item.Commission_Type + "</td><td class= 'text-left text-nowrap fsp-bdr-right fsp-bdr-bottom px-1'>" + item.POSTED_DATE.ToString("dd-MMM-yyyy") + "</td><td class= 'text-right fsp-bdr-right fsp-bdr-bottom px-1'>" + Utility.FormatCurrency(item.Display_Amount) + "</td><td class= 'text-left fsp-bdr-bottom px-1'><a href ='https://www.google.com/' target ='_blank'><img class='leftarrowlogo' src='assets/images/leftarrowlogo.png' alt='Left Arrow'></a></td></tr>");
